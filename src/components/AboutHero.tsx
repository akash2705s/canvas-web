...
function StatCard({ value, label, sub, valueColor }: StatCardProps) {
  return (
    <motion.div
      ...
      className="relative overflow-hidden rounded-2xl border border-white/10 bg-white/[0.06] shadow-[0_20px_60px_rgba(0,0,0,0.7)] backdrop-blur-xl"
    >
      <div className="relative flex h-full flex-col justify-between rounded-2xl bg-transparent px-4 py-4">
        <div className="mt-1">
          <p
            className="font-extrabold text-[32px] leading-[32px] tracking-[0px]"
            style={{ color: valueColor }}
          >
            {value}
          </p>
          <p className="mt-2 font-semibold text-[13px] leading-[18px] tracking-[0px] text-white">
            {label}
          </p>
        </div>
        <p className="mt-1 font-normal text-[11px] leading-[15px] tracking-[0px] text-[#FFFFFF4D]">
          {sub}
        </p>
      </div>
    </motion.div>
