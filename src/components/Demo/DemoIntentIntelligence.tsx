      ...
      <div className="mx-auto max-w-7xl px-6 sm:px-10">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 lg:items-start">
          ...
          <motion.div
            ...
            className="rounded-3xl overflow-hidden shadow-2xl border border-slate-200 bg-white transition-all cursor-pointer"
          >
            <motion.div className="p-6">
              ...
              <motion.div className="space-y-4">
                ...
                >
                  {[
                    { label: "Total intent signals captured", value: "2,189", color: "text-slate-900" },
                    { label: "Interaction rate", value: "26.2%", color: "text-orange-500" },
                    { label: "Avg engagement", value: "14s+", color: "text-blue-600" },
                  ].map((stat) => (
                    <motion.div
                      key={stat.label}
                    ...
                    >
                      ...
                      <motion.p className={`text-3xl font-bold ${stat.color}`} whileHover={{ scale: 1.1 }}>
                        {stat.value}
                      </motion.p>
                    </motion.div>
                  ))}
                ...
                >
                  {[
                    { icon: ctaIcon, title: "CTA Click", desc: "Highest intent CTA at overlay", value: 847, maxValue: 847, color: "text-orange-500", bgColor: "bg-white", label: "CTA" },
                    { icon: qrcIcon, title: "QR Scan", desc: "Viewer scanned code with mobile device", value: 612, maxValue: 847, color: "text-blue-500", bgColor: "bg-white", label: "QR" },
                    { icon: productIcon, title: "Product Explore", desc: "Viewer opened expandable product card", value: 439, maxValue: 847, color: "text-purple-500", bgColor: "bg-white", label: "Product" },
                    { icon: saveLaterIcon, title: "Save for Later", desc: "Viewer saved items to their watch list", value: 291, maxValue: 847, color: "text-yellow-500", bgColor: "bg-white", label: "Save" },
                  ].map((signal, idx) => (
                    <motion.div
                      key={signal.label}
                      variants={{
                      ...
                      <div className="flex items-start gap-3 mb-1.5">
                        ...
                        <div className="flex-1">
                          ...
                          <motion.div className="flex items-start justify-between gap-3 mb-1" whileHover={{ x: 2 }}>
                            ...
                            <motion.span className={`text-sm font-bold ${signal.color} shrink-0 cursor-pointer`} whileHover={{ scale: 1.2 }}>
                              {signal.value}
                            </motion.span>
                          </motion.div>
                          <motion.div className="h-2 bg-slate-200 rounded-full overflow-hidden cursor-pointer" whileHover={{ boxShadow: "0 0 8px rgba(0,0,0,0.1)" }}>
                            <motion.div
                              className={`h-full rounded-full transition-all ${signal.color.replace('text-', 'bg-')}`}
                              initial={{ width: 0 }}
                              whileInView={{ width: `${(signal.value / signal.maxValue) * 100}%` }}
                              viewport={{ once: true }}
                              transition={{ duration: 0.8, delay: idx * 0.1 }}
                              whileHover={{ boxShadow: `inset 0 0 8px rgba(0,0,0,0.1)` }}
                            />
                          </motion.div>
                        </div>
                      </div>
